...
import { updateFoodDetailsFormSchema } from "./edit-food-form-schema";
import {
  updateFood,
  UpdateFoodDetails,
} from "@/actions/add-food/UpdateFoodDetails.action";

const UpdateFoodDetailsFormSheetComponent = ({
  children,
  data,
}: {
  ...
  const form = useForm<z.infer<typeof updateFoodDetailsFormSchema>>({
    resolver: zodResolver(updateFoodDetailsFormSchema),
    defaultValues: {
      name: data?.name,
      mrp: String(data?.price?.mrp),
      restaurantShare: String(data?.price?.restaurantShare),
      companyShare: String(data?.price?.companyShare),
      category: data?.category._id,
      description: data?.description,
      restaurant: data?.restaurant?._id,
      vegOrNonVeg: data?.vegOrNonVeg.toLowerCase(),
      image: data?.image,
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof updateFoodDetailsFormSchema>) {
    setLoading(true);
    ...
    try {
      ...
      const response = await UpdateFoodDetails(data._id, dataToBeSent);
  ...
  return (
    <Sheet>
      ...
      <SheetContent className="light:bg-white overflow-scroll">
        <SheetHeader>
          <SheetTitle>Edit Food Details</SheetTitle>
          <SheetDescription>
            Edit food details and submit the form.
          </SheetDescription>
        </SheetHeader>
        <div className="mt-16">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
              <section>
                <div className="self-end justify-self-end -translate-y-1.5">
                  <section className="relative grid justify-center border border-gray-400 dark:border-gray-700 rounded-md w-full h-[15rem] cursor-not-allowed">
                    {imagePreview ? (
                      <div className="w-full h-full overflow-hidden rounded-md">
                        <Image
                          ...
                          src={imagePreview}
                          alt="Image preview"
                          className="w-full h-full object-fill object-bottom aspect-video block"
                        />
                      </div>
                    ) : (
                      ...
                      <FormField
                        ...
                        render={() => (
                          <FormItem>
                            <FormControl>
                              <div
                                className={`border w-full h-11 rounded-md flex items-center relative text-gray-500 dark:text-gray-400 overflow-scroll border-gray-400 dark:border-gray-700`}>
                                    ...
                                    <>
                                      {restaurants?.map((restaurant) => (
                                        <SelectItem
                                          ...
                                          value={restaurant?.docId} // Use restaurant ID
                                        >
...
export default UpdateFoodDetailsFormSheetComponent;
